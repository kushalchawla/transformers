...
class TranslationDataset(data.Dataset):
    ...
    def __init__(self, inp_file, targ_file, inp_tokenizer, targ_tokenizer):
        """
        Assume max len to be 100: basically this is a single utterance. ..yeah 100 is too much I think. even 50 is good. .but lets keep it to 100.
        """
        self.inp_tokenizer = inp_tokenizer
        self.targ_tokenizer = targ_tokenizer

        max_length = 100 #common for encoder-decoder.
        ...
        with open(inp_file, "r") as ef:
            for line in ef:
                enc = self.inp_tokenizer.encode(line.strip(), add_special_tokens=True, max_length=max_length, truncation=True)
        ...
        with open(targ_file, "r") as df:
            for line in df:
                enc = self.targ_tokenizer.encode(line.strip(), add_special_tokens=True, max_length=max_length, truncation=True)
